...
The `substrate.schema` RPC method now returns a **shallow schema** - the plugin's own schema with child namespaces listed as strings, not full recursive schemas.
...
To get a child's schema, use `substrate.call`:

```json
{"method": "substrate.call", "params": {"method": "solar.schema"}}
```
...
The `params` field in `substrate.call` is now **optional**:

```json
// Both work:
{"method": "substrate.call", "params": {"method": "health.check"}}
{"method": "substrate.call", "params": {"method": "health.check", "params": {}}}
```
...
> {"jsonrpc":"2.0","id":1,"method":"substrate.schema","params":{}}
```
...
- **Breaking:** `substrate.schema` response shape changed - `children` is now `ChildSummary[]` not `PluginSchema[]`
- Each child summary includes `namespace`, `description`, and `hash` (for cache invalidation)
- **Non-breaking:** `substrate.call` params now optional
