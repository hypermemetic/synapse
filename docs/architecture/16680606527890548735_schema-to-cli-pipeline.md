...
### Layer 1: Discovery Schema (substrate.schema)

**Purpose**: Enumerate all activations and their methods

```haskell
-- Returned by substrate.schema RPC call
...
### Layer 2A: Parameter Schema (substrate.full_schema) - Current Approach

**Purpose**: Get typed parameter information for generating CLI flags

```haskell
-- Returned by substrate.full_schema RPC call
...
### Layer 2B: Method Enum Schema (substrate.activation_schema) - Legacy

**Purpose**: Get full JSON Schema with oneOf variants

```haskell
-- Returned by substrate.activation_schema RPC call
...
└─────────────────────────────────────────────────────────────────────┘
          │
          │ RPC: substrate.schema → PlexusSchema
          ↓
    ┌──────────────┐
...
└─────────────────────────────────────────────────────────────────────┘
          ...
          │   RPC: substrate.full_schema(namespace) → ActivationFullSchema
          ↓
    ┌────────────────────────┐
...
- **substrate.schema**: Lightweight, lists all methods (used for activation-level CLI)
- **substrate.full_schema**: Rich, includes param/return schemas (used for typed flags)
...
| `"array"` | `ParamArray t` | repeated flags (e.g., --tags val1 --tags val2) | `TEXT...` |
...
1. **Discovery**: JSON-RPC (`substrate.schema`) → Haskell (`PlexusSchema`)
2. **Enrichment**: JSON-RPC (`substrate.full_schema`) → Haskell (`ActivationFullSchema`)
