...
The `substrate.full_schema` endpoint returns complete method schemas including parameter types and return (stream event) types. This enables clients to understand both what to send and what to expect back.

## RPC Call

```json
{"jsonrpc":"2.0","id":1,"method":"substrate.full_schema","params":["<namespace>"]}
```
...
**Request:**
```json
{"jsonrpc":"2.0","id":1,"method":"substrate.full_schema","params":["bash"]}
```
...
## Comparison with substrate.activation_schema

| Aspect | `substrate.activation_schema` | `substrate.full_schema` |
|--------|---------------------------|---------------------|
...
(echo '{"jsonrpc":"2.0","id":1,"method":"substrate.full_schema","params":["bash"]}'; sleep 1) | websocat ws://localhost:4444
```
