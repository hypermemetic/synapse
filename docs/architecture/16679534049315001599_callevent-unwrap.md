...
After unifying `substrate.call` with the streaming pattern (see `16679536519907867647_plexus-call-streaming.md`), responses from `substrate.call` are now wrapped in `CallEvent`:
...
{"content": {"content": {...}, "content_type": "echo.once", "type": "data"}, "content_type": "substrate.call", "type": "data"}
```
...
unwrapPlexusCallItem item@(StreamData hash prov ct content)
  | ct == "substrate.call" = case fromJSON content of
