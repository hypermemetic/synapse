# Unify substrate.call with Streaming Pattern

## Problem

Currently `substrate.call` is a special case:
...
Make `substrate.call` return a stream like everything else:
...
1. Any code using `substrate.call` needs to handle `CallEvent` instead of raw `PlexusStreamItem`
