...
- `synapse substrate` — shows substrate root schema
- `synapse substrate cone list` — navigates to `cone.list` method
...
This allows `synapse substrate substrate.cone` to work (strips redundant "substrate").
...
// Future (generic entry point, backend-agnostic)
...
// Returns: ["substrate", "agenthub", "toolhub"]
```
...
- Backend-namespaced RPC methods (substrate.call, substrate.schema)
