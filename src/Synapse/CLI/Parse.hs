...
buildParamValue ir param kvs = case pdType param of
  ...
  RefArray innerType ->
    -- Collect all repeated --key value entries (no comma splitting - use shell separation)
...
buildFromTypeDef ir paramName TypeDef{..} kvs = case tdKind of
  ...
  KindAlias target ->
    -- Follow the alias
    buildParamValue ir ParamDef{pdName = paramName, pdType = target, pdDescription = Nothing, pdRequired = True, pdDefault = Nothing, pdDeprecation = Nothing, pdSource = Nothing} kvs
