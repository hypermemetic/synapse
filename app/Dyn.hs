...
main = do
  ...
  globalOpts <- case execParserPure defaultPrefs globalOptsInfo globalArgs of
    ...
    Failure err  -> do
      let (msg, _) = renderFailure err "semantics"
              ...
              Just fullSchema -> do
                ...
                case find (\m -> methodInfoName m == methodName) methods of
                  ...
                  Just methodInfo -> do
                    ...
                    exitSuccess
      _ -> do
        hPutStrLn stderr "Usage: semantics --schema NAMESPACE [METHOD]"
        hPutStrLn stderr "Examples:"
        hPutStrLn stderr "  semantics --schema arbor           # Show all arbor methods"
        hPutStrLn stderr "  semantics --schema arbor tree-list # Show specific method"
        exitFailure
  ...
  when (optTemplate globalOpts) $ do
    case remaining of
      [ns, method] -> do
        ...
        exitSuccess
      _ -> do
        hPutStrLn stderr "Usage: semantics --template NAMESPACE METHOD"
        hPutStrLn stderr "Example: semantics --template arbor tree-list"
        exitFailure
  ...
  when (null remaining || remaining == ["--help"] || remaining == ["-h"]) $ do
    ...
    case cacheResult of
      Left err -> do
        hPutStrLn stderr $ "Failed to load schema: " <> T.unpack err
        putStrLn ""
        putStrLn "Usage: semantics [GLOBAL_OPTIONS] COMMAND [ARGS...]"
        putStrLn ""
        ...
        exitFailure
      Right cached -> do
        putStrLn "semantics - Dynamic CLI for Plexus"
        putStrLn ""
        putStrLn "Usage: semantics [GLOBAL_OPTIONS] COMMAND [ARGS...]"
        putStrLn ""
        putStrLn "Global options:"
  ...
  invocation <- case execParserPure defaultPrefs dynamicInfo remaining of
    ...
    Failure _ -> do
      ...
      case execParserPure defaultPrefs dynamicInfo (remaining ++ ["--help"]) of
        Failure helpErr -> do
          let (msg, _) = renderFailure helpErr "semantics"
          putStrLn msg
        _ -> pure ()
      exitFailure
    CompletionInvoked _ -> exitFailure
  ...
  case itemErrorType of
    ...
    MethodNotFound activation method ->
      let availMethods = case itemAvailableMethods of
            ...
            Nothing -> "Run 'semantics " <> activation <> " --help' to see available methods"
      in T.unlines
...
formatGuidance _ = "Try: semantics --help"

-- | Format suggestion into actionable CLI command
formatSuggestion :: GuidanceSuggestion -> Text
formatSuggestion CallPlexusSchema = "Try: semantics --help"
formatSuggestion (CallActivationSchema namespace) = "Try: semantics " <> namespace <> " --help"
formatSuggestion (TryMethod method mbParams) =
  case mbParams of
    Just params -> "Try: semantics with method '" <> method <> "' and params: " <> T.pack (show params)
    Nothing -> "Try: semantics " <> method <> " --help"
...
handleInfoCommand opts = do
  ...
  putStrLn "    Commands:"
  putStrLn "      semantics --help         Show full help"
  putStrLn "      semantics cache status   Check schema cache"
  putStrLn "      semantics <activation>   Run activation commands"
  putStrLn ""
...
handleCacheCommand opts args = case args of
  ...
  _ -> do
    hPutStrLn stderr "Usage: semantics cache COMMAND"
    hPutStrLn stderr ""
  ...
  [method, paramsJson] -> do
    ...
    case eitherDecode (LBS.pack paramsJson) of
      Left err -> do
        ...
        hPutStrLn stderr "Example: semantics call plexus_schema '[]'"
        exitFailure
      Right params -> do
        ...
        disconnect conn
  _ -> do
    hPutStrLn stderr "Usage: semantics call METHOD PARAMS_JSON"
    hPutStrLn stderr ""
    hPutStrLn stderr "Examples:"
    hPutStrLn stderr "  semantics call plexus_schema '[]'"
    hPutStrLn stderr "  semantics call plexus_activation_schema '[\"cone\"]'"
    hPutStrLn stderr "  semantics call plexus_hash '[]'"
    hPutStrLn stderr "  semantics call arbor_tree_list '{}'"
    hPutStrLn stderr "  semantics call cone_chat '{\"identifier\":{\"by_name\":{\"name\":\"test\"}},\"prompt\":\"hi\"}'"
