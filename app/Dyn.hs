              ...
              Just fullSchema -> do
                ...
                case find (\m -> methodInfoName m == methodName) methods of
                  ...
                  Just methodInfo -> do
                    ...
                    exitSuccess
      _ -> do
        hPutStrLn stderr "Usage: synapse --schema NAMESPACE [METHOD]"
        hPutStrLn stderr "Examples:"
        hPutStrLn stderr "  synapse --schema arbor           # Show all arbor methods"
        hPutStrLn stderr "  synapse --schema arbor tree-list # Show specific method"
        exitFailure
  ...
  when (optTemplate globalOpts) $ do
    case remaining of
      [ns, method] -> do
        ...
        exitSuccess
      _ -> do
        hPutStrLn stderr "Usage: synapse --template NAMESPACE METHOD"
        hPutStrLn stderr "Example: synapse --template arbor tree-list"
        exitFailure
  ...
  when (null remaining || remaining == ["--help"] || remaining == ["-h"]) $ do
    ...
    case cacheResult of
      Left err -> do
        hPutStrLn stderr $ "Failed to load schema: " <> T.unpack err
        putStrLn ""
        putStrLn "Usage: synapse [GLOBAL_OPTIONS] COMMAND [ARGS...]"
        putStrLn ""
        ...
        exitFailure
      Right cached -> do
        putStrLn "synapse - Dynamic CLI for Plexus"
        putStrLn ""
        putStrLn "Usage: synapse [GLOBAL_OPTIONS] COMMAND [ARGS...]"
        putStrLn ""
        putStrLn "Global options:"
  ...
  case itemErrorType of
    ...
    MethodNotFound activation method ->
      let availMethods = case itemAvailableMethods of
            ...
            Nothing -> "Run 'synapse " <> activation <> " --help' to see available methods"
      in T.unlines
...
formatGuidance _ = "Try: synapse --help"

-- | Format suggestion into actionable CLI command
formatSuggestion :: GuidanceSuggestion -> Text
formatSuggestion CallPlexusSchema = "Try: synapse --help"
formatSuggestion (CallActivationSchema namespace) = "Try: synapse " <> namespace <> " --help"
formatSuggestion (TryMethod method mbParams) =
  case mbParams of
    Just params -> "Try: synapse with method '" <> method <> "' and params: " <> T.pack (show params)
    Nothing -> "Try: synapse " <> method <> " --help"
...
handleInfoCommand opts = do
  ...
  putStrLn "    Commands:"
  putStrLn "      synapse --help         Show full help"
  putStrLn "      synapse cache status   Check schema cache"
  putStrLn "      synapse <activation>   Run activation commands"
  putStrLn ""
...
handleCacheCommand opts args = case args of
  ...
  _ -> do
    hPutStrLn stderr "Usage: synapse cache COMMAND"
    hPutStrLn stderr ""
  ...
  [method, paramsJson] -> do
    ...
    case eitherDecode (LBS.pack paramsJson) of
      Left err -> do
        ...
        hPutStrLn stderr "Example: synapse call plexus_schema '[]'"
        exitFailure
      Right params -> do
        ...
        disconnect conn
  _ -> do
    hPutStrLn stderr "Usage: synapse call METHOD PARAMS_JSON"
    hPutStrLn stderr ""
    hPutStrLn stderr "Examples:"
    hPutStrLn stderr "  synapse call plexus_schema '[]'"
    hPutStrLn stderr "  synapse call plexus_activation_schema '[\"cone\"]'"
    hPutStrLn stderr "  synapse call plexus_hash '[]'"
    hPutStrLn stderr "  synapse call arbor_tree_list '{}'"
    hPutStrLn stderr "  synapse call cone_chat '{\"identifier\":{\"by_name\":{\"name\":\"test\"}},\"prompt\":\"hi\"}'"
