...
sampleReadToolUse = [aesonQQ|{
  ...
  "tool_input": {
    "file_path": "/Users/user/dev/controlflow/substrate/Cargo.toml",
    "tool_name": "Read"
  },
  "tool_name": "Read",
...
sampleBashToolUse = [aesonQQ|{
  ...
  "tool_input": {
    "command": "ls -la /Users/user/dev/controlflow/substrate",
    ...
    it "renders Read tool use correctly" $ do
      case renderValue template sampleReadToolUse of
        Right result -> do
          ...
          T.unpack result `shouldContain` "/Users/user/dev/controlflow/substrate/Cargo.toml"
        Left err -> expectationFailure $ "Render failed: " <> show err
    ...
    it "renders Bash tool use correctly" $ do
      case renderValue template sampleBashToolUse of
        Right result -> do
          ...
          T.unpack result `shouldContain` "\"ls -la /Users/user/dev/controlflow/substrate\""
        Left err -> expectationFailure $ "Render failed: " <> show err
